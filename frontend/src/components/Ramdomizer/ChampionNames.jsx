...
const ChampionNames = ({name, setName, random=true}) => {
  ...
  return (
    <div className='name-select'>
        ...
        > <MenuItem value={'ANY'}>ANY</MenuItem>
         ...
         {
            allNames.map(name => 
            <MenuItem value={name.name} key={name.name} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}> {name.name} <img  style={{height: '33px', objectFit:'contain'}}src={`/champions/${name.name.replaceAll(/['\s]/g, '')}.jpg`} /></MenuItem>
            )
        }
